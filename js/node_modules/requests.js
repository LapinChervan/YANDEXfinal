                        ...
                        collection.update({name: user},
                            ...
                            function(err, result) {
                                if (!err) {
                                    ...
                                    db.close();
                                    res.writeHead(200, {'Content-Type': 'text/plain; charset=utf8', 'Access-Control-Allow-Origin': '*'});
                                    res.end('CatFor__' + type + '__IsWrite');
                                }
                            });
                    }
                });
                        ...
                        collection.update({name: data.login},
                            ...
                            function(err, result) {
                                if (!err) {
                                    db.close();
                                    res.writeHead(200, {'Content-Type': 'text/plain; charset=utf8', 'Access-Control-Allow-Origin': '*'});
                                    res.end('Cat_' + data.type + '__IsReWrite');
                                }
                            });
                    }
                });
        ...
        db.open(function(err, db) {
            if (!err) {
                db.collection('users', function(err, collection) {
                    if (!err) {
                        collection.update({name: user},
                            {$addToSet: getObj('history.' + type, data)},
                            function(err, result) {
                                if (!err) {
                                    //  allUsers[user].newCategory(type, cat);
                                    db.close();
                                    res.writeHead(200, {'Content-Type': 'text/plain; charset=utf8', 'Access-Control-Allow-Origin': '*'});
                                    res.end('new__' + type + '__isWrite');
                                }
                            });
                    }
                });
