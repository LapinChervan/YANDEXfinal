                ...
                if (!err) {
                    collection.findOne({name: user }, {fields: { mainCurr: 1, currency: 1, _id: 0}}, function(err, docs) {
                        if (!err) {
                            ...
                            collection.update({name: user}, {$set: {currency: setMainCurr(JSON.parse(valuta))}},
                                function(err, result) {
                                if(!err) {
                                    db.close();
                                    res.end('1');
                        ...
                        if (!err) {
                            ...
                            if (account === 'all') {
                                if (type === 'all') {
                                    for (key in history) {
                                        obj[key] = [];
                                        for (key2 in history[key]) {
                                            obj[key].push(history[key][key2]);
                                        }
                                    }
                                }
                                else {
                                    obj[type] = [];
                                    for (key in history[type]) {
                                        obj[type].push(history[type][key]);
                                    }
                                }
                    ...
                    collection.findOne({session: sha}, function(err, docs) {
                        if (!err && docs !== null) {
                            db.collection('users', function(err, collection1) {
                                if (!err) {
                                    collection1.findOne({name: docs.login}, {fields: { total: 0, password: 0, _id: 0} },
                                        function(err, docs) {
                                            if (!err && docs !== null) {
                                                db.close();
                                                res.writeHead(200, {'Content-Type': 'text/html; charset=utf-8'});
