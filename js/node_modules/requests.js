...
requests = (function() {
    ...
    */
                        ...
                        if (!err) {
                            function setMainCurr(dataCurr) {
                                ...
                                for (key in mainValues) {
                                    elem = currency[key];
                                    //valuta = Math.round(1 / mainValues[key] * 100) / 100;
                                    valuta = 1 / mainValues[key];
                                    elem[mainCurr] = valuta;
                                    for (j in elem) {
                                        if (j !== mainCurr) {
                                            //elem[j] = Math.round(valuta * mainValues[j] * 100) / 100;
    ...
    /**
    * Создание динамического объекта для передачи параметра как свойства при записи в ДОКУМЕНТ.
    *
